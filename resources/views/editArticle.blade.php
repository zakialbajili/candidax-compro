...
<main class="w-full h-screen flex items-center justify-center font-poppins text-primerText pt-[200px]">
    <div class="w-full h-fit rounded-xl flex flex-col gap-10 px-[5%]">
        <a
            ...
            <x-icons.arrowLeft />
            <p class="text-2xl">Edit Article</p>
        </a>
        <form action="{{route('admin.editArticle', $detailArticle['id'])}}" method="POST" enctype="multipart/form-data" class="w-full bg-subtleGray flex gap-20 py-10 px-[5%]">
            @csrf
            @method('put')
            <div class="w-[332px] h-[418px] flex flex-col items-center justify-center border-2 border-dashed border-border">
                <!-- Preview Image -->
                @if(!empty($detailArticle['foto']))
                <div id="containerPreviewDB" class=" w-full h-full relative">
                    <img id="previewDB" src="{{asset('/storage/image/'.$detailArticle['foto'])}}" class="w-full h-full object-cover rounded-lg" />
            ...
            <div class="flex flex-grow flex-col gap-5 text-xs text-primerText">
                ...
                <div class="w-full flex flex-col gap-[10px]">
                    ...
                    <textarea
                        ...
                        class="py-[10px] px-3 rounded-xl border-2 border-primerText h-[150px]">{{$detailArticle['description']}}</textarea>
                    @error('description')
                    <p class="text-cherryRed">{{$message}}</p>
                    @enderror
                </div>
