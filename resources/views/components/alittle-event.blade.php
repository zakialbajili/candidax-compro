    ...
    <div class="w-full flex justify-between gap-5 lg:gap-16 overflow-x-scroll lg:overflow-x-hidden">
        @if(!empty($listEvents[0]))
            <div class="w-full lg:w-[50%] flex flex-col lg:flex-row justify-between gap-[26px]">
                <img src="{{ asset('/storage/image/'.$listEvents[0]->foto) }}" alt="Article Thumbnail" class="w-fit h-fit">
        ...
        @for($i = 1; $i < count($listEvents); $i++)
            <div class="flex flex-col lg:flex-row gap-x-5 gap-y-[26px]">
                <img src="{{ asset('/storage/image/'.$listEvents[$i]->foto) }}" alt="Article Thumbnail" class="w-fit h-fit lg:w-[170px] lg:h-[212px]">
