...
@section('title', 'Add Event | Candidax')
@section('content')
<main class="w-full h-screen flex items-center justify-center font-poppins text-primerText pt-[200px]">
    <div class="w-full h-fit rounded-xl flex flex-col gap-10 px-[5%]">
        <a
            ...
            <x-icons.arrowLeft />
            <p class="text-2xl">Add Event</p>
        </a>
        <form action="{{route('admin.createEvent')}}" method="POST" enctype="multipart/form-data" class="w-full bg-subtleGray flex gap-20 py-10 px-[5%]">
            @csrf
            ...
            <div class="flex flex-grow flex-col gap-5 text-xs text-primerText">
                ...
                <div class="w-full flex flex-col gap-[10px]">
                    ...
                    <textarea
                        ...
                        id="description"
                        placeholder="Masukkan content"
                        class="py-[10px] px-3 rounded-xl border-2 border-primerText min-h-[150px]"></textarea>
                    @error('description')
                    <p class="text-cherryRed">{{$message}}</p>
                    @enderror
                </div>
